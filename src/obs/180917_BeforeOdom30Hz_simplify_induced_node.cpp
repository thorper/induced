...
class CorrData
{
	...
	public:
		...
		CorrData()
		{
			...
			gpl_sub.subscribe(nh,"/mavros/global_position/local", 100);
		...
		void callback(const ImuConstPtr& imu, const OdometryConstPtr& gpl, const WindStampedConstPtr& wind)
		{
			...
			Vector3d OmegaG1(imu->angular_velocity.x,imu->angular_velocity.y,imu->angular_velocity.z);
			...
			// Vector3d DAwind(wind->u,wind->v,wind->w); //wind data now comes in {2,NED} not {A,DSW}
