...
class CorrData
{
	...
	public:
		...
		CorrData()
		{
			...
			lpv_sub.subscribe(nh,"/mavros/local_position/velocity",50);
			wind_sub.subscribe(nh, "/wind_data", 100);

			wind_pub_corr = nh.advertise<WindStamped>("wind_data_corrected",50);

 			sync.reset(new Sync(MySyncPolicy(100),lpo_sub, lpv_sub, wind_sub));
  			sync->registerCallback(boost::bind(&CorrData::callback,this,_1, _2, _3));

		}

		void callback(const OdometryConstPtr& lpo, const TwistStampedConstPtr& lpv, const WindStampedConstPtr& wind)
		{
			...
			Vector3d VS1(lpv->twist.linear.x,lpv->twist.linear.y,lpv->twist.linear.z);
	...
	private:
		...
		message_filters::Subscriber<TwistStamped> lpv_sub;
 		message_filters::Subscriber<WindStamped> wind_sub;

		typedef sync_policies::ApproximateTime<Odometry,TwistStamped,WindStamped> MySyncPolicy;
